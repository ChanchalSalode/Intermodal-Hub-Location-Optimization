...
import time
import matplotlib; matplotlib.use("Agg")  # headless backend to avoid GUI/memory issues
...
def pick_col(cols, *names):
    """Utility: pick the first matching column name (case-insensitive)."""
...
# Precompute A_k once
A_by_k = {k: [] for k in K_ids}
for (ih, jh, kk) in A:
    A_by_k[kk].append((ih, jh))

print("Precomputed A_k structure.")
...
eta = mdl.addVars(K_ids, lb=0.0, vtype=GRB.CONTINUOUS, name="eta")

mdl.setObjective(quicksum(eta[k] for k in K_ids), GRB.MINIMIZE)
mdl.addConstr(quicksum(y[i] for i in H_hubs) == p, name="hub_limit")

# ==========================================================
# PHASE 1 — LOOPED CLASSICAL BENDERS (LP MASTER, DUAL-BASED)
# ==========================================================
print("\n==============================")
print("PHASE 1: Classical Looped LP Benders (Dual-based)")
print("==============================")

# Relax y
for i in H_hubs:
    y[i].vtype = GRB.CONTINUOUS
mdl.update()
...
# ===============================
# Core-point initialization (Magnanti-Wong)
# ===============================
lambda_core = 0.5   # 0.3–0.7 works well
y_bar = {i: 1.0/len(H_hubs) for i in H_hubs}  # strictly interior

# Precompute routing costs per commodity (like basic implementation)
...
for (ih, jh, k) in A:
    ...
    paths_by_k[k].append((ih, jh, cost))
...
while iteration < MAX_ITERS:
    ...
    if mdl.Status != GRB.OPTIMAL:
        raise RuntimeError("LP Master not optimal.")

    yhat = {i: y[i].X for i in H_hubs}
    LB = mdl.ObjVal
    # ===============================
    # Core-point update
    # ===============================
    for i in H_hubs:
        y_bar[i] = lambda_core * y_bar[i] + (1 - lambda_core) * yhat[i]

    # Keep strictly interior (avoid 0 or 1)
    eps_core = 1e-4
    for i in H_hubs:
        y_bar[i] = max(eps_core, min(1 - eps_core, y_bar[i]))

    total_subproblem_value = 0.0
    cuts_added = 0

    # ----- Solve dual subproblem for each commodity -----
    for k in K_ids:

        dual = Model(f"dual_k{k}")
        dual.Params.OutputFlag = 0

        # Dual variables
        pi = dual.addVar(lb=-GRB.INFINITY, name="pi")
        mu = dual.addVars(H_hubs, lb=0.0, name="mu")

        # Dual constraints:
        # pi - mu_i - mu_j <= c_ijk
        for (ih, jh, cost) in paths_by_k[k]:
            dual.addConstr(
                pi - mu[ih] - mu[jh] <= cost
            )
        # Dual objective
        dual.setObjective(
            pi - quicksum(mu[i] * yhat[i] for i in H_hubs),
            GRB.MAXIMIZE
        )
        dual.optimize()

        if dual.Status != GRB.OPTIMAL:
            raise RuntimeError(f"Dual infeasible for commodity {k}")
        dual_val = dual.ObjVal
        total_subproblem_value += dual_val

        pareto = Model(f"pareto_k{k}")
        pareto.Params.OutputFlag = 0

        pi_p = pareto.addVar(lb=-GRB.INFINITY)
        mu_p = pareto.addVars(H_hubs, lb=0.0)

        # same dual feasibility
        for (ih, jh, cost) in paths_by_k[k]:
            pareto.addConstr(
                pi_p - mu_p[ih] - mu_p[jh] <= cost
            )

        # force same objective at current yhat
        pareto.addConstr(
            pi_p - quicksum(mu_p[i] * yhat[i] for i in H_hubs) == dual_val
        )

        # maximize at interior point
        pareto.setObjective(
            pi_p - quicksum(mu_p[i] * y_bar[i] for i in H_hubs),
            GRB.MAXIMIZE
        )

        pareto.optimize()

        if pareto.Status == GRB.OPTIMAL:
            pi_val = pi_p.X
            mu_vals = {i: mu_p[i].X for i in H_hubs}
        else:
            # fallback to classical
            pi_val = pi.X
            mu_vals = {i: mu[i].X for i in H_hubs}

        # ----- Add classical optimality cut -----
        if eta[k].X < dual_val - EPS:
            mdl.addConstr(
                eta[k] >= pi_val - quicksum(mu_vals[i] * y[i] for i in H_hubs)
            )
            saved_cuts.append((k, pi_val, mu_vals))
            cuts_added += 1

    print(f"LB={LB:.6f}, Subproblem={total_subproblem_value:.6f}, cuts={cuts_added}")

    if cuts_added == 0:
        print("Phase-1 converged.")
        break

print(f"Total Phase-1 cuts: {len(saved_cuts)}")

# Heuristic integer solution
sorted_hubs = sorted(H_hubs, key=lambda i: y[i].X, reverse=True)
heuristic_open = sorted_hubs[:p]

UB_estimate = 0.0
for k in K_ids:
    best = float("inf")
    for (ih, jh) in A_by_k[k]:
        if ih in heuristic_open and jh in heuristic_open:
            cost_val = wk[k] * (
                c_ik[(ok[k], ih)]
                + alpha_km * c_km[(ih, jh)]
                + cost_mj(jh, dk[k])
...
print("Heuristic UB:", UB_estimate)

# ==========================================================
# PHASE 2 — RESTORE BINARY + PRELOAD CUTS
# ==========================================================
print("\n==============================")
print("PHASE 2: BBC with Saved Cuts")
print("==============================")
...
# Re-add saved cuts
for (k, pi_val, mu_dict) in saved_cuts:
    mdl.addConstr(
        eta[k] >= pi_val - quicksum(mu_dict[i] * y[i] for i in H_hubs)
    )

mdl.update()

# ==========================================================
# BENDERS CALLBACK (Phase 2)
# ==========================================================
def benders_callback(model, where):
    ...
    yhat = {i: model.cbGetSolution(y[i]) for i in H_hubs}
    eta_hat = {k: model.cbGetSolution(eta[k]) for k in K_ids}

    open_hubs = [i for i in H_hubs if yhat[i] > 0.5]
    ...
    for k in K_ids:
        ...
        for (ih, jh) in A_by_k[k]:
            if ih in open_hubs and jh in open_hubs:
                ...
                best_cost = min(best_cost, cost_val)

        if best_cost == float("inf"):
            continue

        # Solve dual subproblem (same as Phase-1)
        dual = Model()
        dual.Params.OutputFlag = 0

        pi = dual.addVar(lb=-GRB.INFINITY)
        mu = dual.addVars(H_hubs, lb=0.0)

        for (ih, jh, cost) in paths_by_k[k]:
            dual.addConstr(pi - mu[ih] - mu[jh] <= cost)
...
for k in K_ids:
    ...
    xk = sub.addVars(A_k, lb=0.0, ub=1.0, vtype=GRB.CONTINUOUS)
    ...
    sub.addConstr(quicksum(xk[ih, jh] for (ih, jh) in A_k) == 1)

    for i in H_hubs:
        sub.addConstr(
            quicksum(xk[ih, jh] for (ih, jh) in A_k if ih == i)
            + quicksum(xk[jh, ih] for (jh, ih) in A_k if ih == i and jh != i)
            <= y[i].X
        )
...
for key, val in x_sol.items():
    if val > 1e-8:
        ih, jh, k = key
        print(f"x[i_hub={ih}, j_hub={jh}, k={(ok[k], dk[k])}] = {val:.0f}")


# --- Hub–hub arc usage summary (across all ODs) ---
from collections import defaultdict as _dd
arc_flow = _dd(float)
for (ih, jh, k), val in x_sol.items():
    if val > 1e-9:
        arc_flow[(ih, jh)] += wk[k]
...
with open(output_file, "w") as f:
    ...
    # for (ih, jh, k), val in x_sol.items():
    #     if val > 0.5:
    #         f.write(f"x[i_hub={ih}, j_hub={jh}, k=({ok[k]}, {dk[k]})] = {val:.0f}\n")
...
SAVE_PLOT_PATH = 'pictorial_representation_BBC.png'
...
if DO_PLOT:
    try:
        ...
        if not (nid and nlat and nlon):
            ...
            flows = []
            for (ih, jh, k), val in x_sol.items():
                if val >= PLOT_THRESHOLD:
                    flows.append({'o': ok[k], 'd': dk[k], 'i': ih, 'j': jh, 'share': float(val)})
...
opened_df.to_csv("opened_hubs_BBC.csv", index=False)
